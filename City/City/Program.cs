...
static void AdoNET()
{
    ...
    //while (reader.Read())
    //{
    //    var connection_del = new SqlConnection(@"Server = MSI; Database = City2; Trusted_Connection = True;");
    //    connection_del.Open();
    ...
    command = connection.CreateCommand();
    command.CommandText = "SELECT a.number, b.name FROM homes a, streets b where a.id_streetID = b.ID";
    reader = command.ExecuteReader();
    while (reader.Read())
    {
    ...
    reader.Close();
    command = connection.CreateCommand();
    command.CommandText = "select top(1) b.ID, a.name, SUM(b.amount) as sum from streets a, homes b where a.ID = b.id_streetID GROUP BY a.name, b.ID order by sum(b.amount)";
    reader = command.ExecuteReader();

    //while (reader.Read())
    //{
    //    Console.WriteLine($"\nНаименее заселённая улица: {reader.GetString(0)}, количество квартир:{reader.GetInt32(reader.GetOrdinal("sum"))}");
    //}
    //reader.Close();
    //var command2 = connection.CreateCommand();
    //command2.CommandText = $"select ID from streets where name = '{reader.GetString(0)}'";
    //reader = command2.ExecuteReader();
    //while (reader.Read())
    //{
    //    Console.WriteLine($"\nНаименее заселённая улица: {reader.GetString(0)}");
    //}
    while (reader.Read())
    {
        var connection_del = new SqlConnection(@"Server = MSI; Database = City2; Trusted_Connection = True;");
        connection_del.Open();
        var command_del = connection_del.CreateCommand();
        command_del.CommandText = $"INSERT INTO homes VALUES(25, {reader.GetInt32(0)}, 300, 2022)";
        command_del.ExecuteReader();
        connection_del.Close();
    }
    reader.Close();
...
static async void ForEF()
{
    ...
    //Console.WriteLine($"1. Список домов, отсортированных по давности ремонта:");
    //foreach (home_new e in db.homes_new.OrderBy(e => e.year_repair))
    //{
    //    Console.WriteLine($"{e.number} {e.id_street.name}");
    //}

    //Console.WriteLine($"2. Список домов, отсортированных по степени потребности в ремонте:");
    //foreach (home_new e in db.homes_new.OrderByDescending(e => ((2022 - e.year_repair) * e.amount)))
    //{
    //    Console.WriteLine($"{e.number} {e.id_street.name}");
    //}
    ...
    //decimal sum = 0;
    //decimal ave = 0;
    //foreach (var e in db.homes_new)
    //{
    //    ave += e.year_repair;
    //    sum++;
    //}
    //foreach (home_new e in db.homes_new.Where(e => e.year_repair < ave / sum))
    //{
    //    Console.WriteLine($"{e.number} {e.id_street.name} {e.year_repair}");
    //}

    //Console.WriteLine($"4. Дома ремонтированные позднее 5-ти лет:");
    //foreach (var g in db.homes_new.Where(e => e.year_repair < 2017).GroupBy(e => e.id_street.name)
    //        .Select(x => new { street_name = x.Key, count = x.Count() }))
    //{
    //    Console.WriteLine($"{g.street_name} {g.count}");
    //}
    ...
    //Console.WriteLine($"5. Самый старый по ремонту дом:");
    //decimal oldHome = 2030;
    //foreach (home_new e in db.homes_new)
    //{
    //    if (e.year_repair < oldHome)
    //    {
    //        oldHome = e.year_repair;
    //    }
    //}
    //foreach (home_new e in db.homes_new.Where(e => e.year_repair == oldHome))
    //{
    //    Console.WriteLine($"{e.number} {e.id_street.name} {e.year_repair}");
    //}

    //Console.WriteLine($"Произвелся ремонт:");
    //Models db2 = new Models();
    //foreach (home_new e in db2.homes_new.Where(e => e.year_repair == oldHome))
    //{
    //    e.year_repair = 2022;
    //    Console.WriteLine($"{e.number} {e.id_street.name} {e.year_repair}");
    //}
    ...
    //Models db3 = new Models();
    //int kv = db3.homes_new.Min(e => Convert.ToInt32(e.amount));
    //foreach (home_new e in db3.homes_new)
    //{
    //    if (e.amount == kv)
    //    {
    //        db3.homes_new.Remove(e);
    //        break;
    //    }
    //}
    //db3.SaveChanges();
    ...
    //Console.WriteLine($"7. Наименее заселенная улица:");
    //Models db4 = new Models();
    //int zasel = db4.homes_new.Min(e => Convert.ToInt32(e.id_street.name.Count()) * Convert.ToInt32(e.amount));
    //{
    //    foreach (home_new e in db4.homes_new)
    //    {
    //        if (Convert.ToInt32(e.id_street.name.Count()) * Convert.ToInt32(e.amount) == zasel)
    //        {
    //            int max_id = db4.homes_new.Count() + 1;
    //            db4.homes_new.Add(new home_new { ID = max_id, number = "33", id_street = e.id_street, amount = 100, year_repair = 2022 });
    //            break;
    //        }
    //    }
    //}
